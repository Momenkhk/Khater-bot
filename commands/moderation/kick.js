...
module.exports = {
  ...
  async execute({ interaction, message, args, source }) {
    ...
    if (!user) return source === 'slash' ? interaction.reply({ content: 'User required.', ephemeral: false }) : message.reply('User required.');
    const member = await (interaction?.guild || message.guild).members.fetch(user.id).catch(() => null);
    if (!member?.kickable) return source === 'slash' ? interaction.reply({ content: 'I cannot kick this user.', ephemeral: false }) : message.reply('I cannot kick this user.');
