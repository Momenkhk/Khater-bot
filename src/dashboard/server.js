...
<style>
:root{--bg:#101322;--panel:#181d33;--panel2:#202743;--line:#2b335a;--txt:#eef2ff;--accent:#4f78ff}
...
.command{display:grid;grid-template-columns:1fr auto auto;gap:8px;align-items:center;background:#131a37;border:1px solid #2d3766;border-radius:12px;padding:10px;margin:8px 0}
input,select{width:100%;background:#0f1430;color:#fff;border:1px solid #364070;border-radius:10px;padding:9px}button{background:var(--accent);color:#fff;border:0;border-radius:10px;padding:9px 12px;cursor:pointer}
.toggle{appearance:none;width:46px;height:26px;border-radius:20px;background:#3a3f61;position:relative;cursor:pointer}.toggle:checked{background:#4f78ff}.toggle:before{content:'';position:absolute;top:3px;right:3px;width:20px;height:20px;background:#fff;border-radius:50%}.toggle:checked:before{right:23px}
...
<section class="section"><h2>الأوامر (الإشراف)</h2><div id="commandList"></div><button id="saveCommands">حفظ الأوامر</button></section>
...
<section class="section"><h2>Whitelist</h2><div class="card"><label>إضافة ID</label><input id="wlAdd"/><label>حذف ID</label><input id="wlRemove"/><div style="display:flex;gap:8px;margin-top:8px"><button id="wlAddBtn">إضافة</button><button id="wlRemoveBtn">حذف</button></div><pre id="wlView"></pre></div></section>
...
function renderCommands(){const wrap=$('commandList');wrap.innerHTML='';commands.forEach((c)=>{const cfg=(settings.commands||{})[c.name]||{enabled:true,aliases:[]};const row=document.createElement('div');row.className='command';row.innerHTML='<strong>'+c.name+'</strong><input class="toggle" type="checkbox" data-cmd="'+c.name+'" '+(cfg.enabled!==false?'checked':'')+'/><button data-edit="'+c.name+'">اختصار</button>';wrap.appendChild(row);});document.querySelectorAll('[data-edit]').forEach((b)=>b.onclick=()=>{const name=b.dataset.edit;const current=((settings.commands||{})[name]?.aliases||[]).join(',');const aliases=prompt('اكتب الاختصارات مفصولة بفاصلة',current);if(aliases===null)return;settings.commands=settings.commands||{};settings.commands[name]=Object.assign({enabled:true},settings.commands[name]||{}, {aliases:aliases.split(',').map(x=>x.trim()).filter(Boolean)});notice('تم تعديل '+name+' (لا تنسى حفظ الأوامر)');});}
